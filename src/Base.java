...
//				contractPane.setForeground(Color.white);
	...
	public base() {
		...
		for (int i = 0; i < mRightButtons.size(); i++) {
			mEffects.add(new JLabel(new ImageIcon("mMouse.png")));
			mEffects.get(i).setBounds(initXCoordinate + i / 3 * 300 + bInsets.left - 5,
					initYCoordinates[i / 3] + i % 3 * 200 + bInsets.top - 5, 280, 190);
			add(mEffects.get(i));
			mEffects.get(i).setVisible(false);
			mRightButtons.get(i).setBounds(initXCoordinate + i / 3 * 300 + bInsets.left,
					initYCoordinates[i / 3] + i % 3 * 200 + bInsets.top, mButtonsDimen.width, mButtonsDimen.height);
			mRightButtons.get(i).addMouseListener(mMousListButtons);
			add(mRightButtons.get(i));
		}
		contractPaneButton.addMouseListener(mMousListButtons);
	...
	/**
	 * The procedure type setter method is used to update information displayed on
	 * base interface
	 * 
	 * @param yourName  updated username
	 * @param lv        updated level number
...
>>>>>>> fcd74addd94d754b4c50e48ab9e2bc3288eca85a:src/base.java
